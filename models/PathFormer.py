...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.projections = nn.Sequential(nn.Linear(self.seq_len * self.d_model, self.pre_len))
        
    def forward(self, x):                       # [64, 96, 21]
        ...
        out = self.start_fc(x.unsqueeze(-1))    # [64, 96, 21, 16]

        for layer in self.AMS_lists:
            out, aux_loss = layer(out)          # [64, 96, 21, 16]
