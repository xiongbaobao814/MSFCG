...
# 时间序列趋势性和季节性分解
    def seasonality_and_trend_decompose(self, x):
        x = x[:, :, :, 0]                           # [64, 96, 21]
        _, trend = self.trend_model(x)              # [64, 96, 21]
    ...
    def forward(self, x, loss_coef=1e-2):
        # Multi-Scale Router之(1.时间序列趋势性和季节性分解)
        new_x = self.seasonality_and_trend_decompose(x)             # [64, 96, 21]

        # Multi-Scale Router之(2.routing)
        gates, load = self.noisy_top_k_gating(new_x, self.training) # [64, 4],[4]
        
        # calculate balance loss
        importance = gates.sum(0)   # [4]
        ...
        # expert_outputs={0:[26, 96, 21, 16],1:[36, 96, 21, 16],2:[37, 96, 21, 16],3:[29, 96, 21, 16]}
        
        # Multi-Scale Aggregator
        output = dispatcher.combine(expert_outputs) # [64, 96, 21, 16]
