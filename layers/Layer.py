...
class Transformer_Layer(nn.Module):
    ...
    def forward(self, x):
        ...
        # weights_distinct={0:[21,16,16],1:[21,16,16]}, biases_distinct={0:[21,16],1:[21,16]}
        
        ####intra Attention#####
        for i in range(self.patch_nums):
            t = x[:, i*self.patch_size:(i+1)*self.patch_size, :, :]   # [26, 16, 21, 16]
            intra_emb = self.embeddings_generator[i](self.intra_embeddings[i]).expand(batch_size, -1, -1,  -1)
            # intra_emb:[26, 1, 21, 16]
            t = torch.cat([intra_emb, t], dim=1)    # [26, 17, 21, 16]
            out, attention = self.intra_patch_attention(intra_emb, t, t, weights_distinct, 
                                                        biases_distinct, weights_shared, biases_shared)
            # out:[26, 1, 21, 16], attention:[52, 21, 1, 17]
