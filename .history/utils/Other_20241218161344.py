...
class FourierLayer(nn.Module):
    ...
    def topk_freq(self, x_freq):
        ...
        index_tuple = (mesh_a.unsqueeze(1), indices, mesh_b.unsqueeze(1)) # {0:[32,1, 7]; 1:[32, 3, 7]; 2:[32, 3, 7]}
    ...
    def forward(self, x): # x:(b, t, d)
        ...
        b, t, d = x.shape                           # 64, 96, 7
        x_freq = fft.rfft(x, dim=1)                 # [64, 49, 7]

        if t % 2 == 0:
            x_freq = x_freq[:, self.low_freq:-1]    # [64, 47, 7]
            f = fft.rfftfreq(t)[self.low_freq:-1]   # [47]
        else:
            x_freq = x_freq[:, self.low_freq:]
            f = fft.rfftfreq(t)[self.low_freq:]

        x_freq, index_tuple = self.topk_freq(x_freq)                            # [64, 3, 7]
        f = repeat(f, 'f -> b f d', b=x_freq.size(0), d=x_freq.size(2))         # [64, 47, 7]
        f = f.to(x_freq.device)
        f = rearrange(f[index_tuple], 'b f d -> b f () d').to(x_freq.device)    # [64, 1, 3, 7]
