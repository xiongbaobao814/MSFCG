...
class MLP(nn.Module):
    def __init__(self, input_size, output_size):
        ...
        self.fc = nn.Conv2d(in_channels=input_size, out_channels=output_size, kernel_size=(1, 1),
                            bias=True)
...
class FourierLayer(nn.Module):
    ...
    def forward(self, x): # x:(b, t, d)
        ...
        b, t, d = x.shape                           # 64, 96, 21
        x_freq = fft.rfft(x, dim=1)                 # [64, 49, 21]

        if t % 2 == 0:
            x_freq = x_freq[:, self.low_freq:-1]    # [64, 47, 21]
            f = fft.rfftfreq(t)[self.low_freq:-1]   # 47
        else:
            x_freq = x_freq[:, self.low_freq:]
            f = fft.rfftfreq(t)[self.low_freq:]

        x_freq, index_tuple = self.topk_freq(x_freq)                            # [64, 3, 21]
        f = repeat(f, 'f -> b f d', b=x_freq.size(0), d=x_freq.size(2))         # [64, 47, 21]
        f = f.to(x_freq.device)
        f = rearrange(f[index_tuple], 'b f d -> b f () d').to(x_freq.device)    # [64, 1, 3, 21]
