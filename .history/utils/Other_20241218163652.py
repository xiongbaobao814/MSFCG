...
class SparseDispatcher(object):
    def __init__(self, num_experts, gates):
        ...
        # sort experts
        sorted_experts, index_sorted_experts = torch.nonzero(gates).sort(0) # [128, 2]
    ...
    def dispatch(self, inp):
        ...
        inp_exp = inp[self._batch_index].squeeze(1)    # [128, 96, 21, 16]
...
class FourierLayer(nn.Module):
    ...
    def topk_freq(self, x_freq):
        ...
        index_tuple = (mesh_a.unsqueeze(1), indices, mesh_b.unsqueeze(1)) # {0:[32,1,7]; 1:[32,3,7]; 2:[32,1,7]}
        x_freq = x_freq[index_tuple]    # [32, 3, 7]
        return x_freq, index_tuple
    
    def forward(self, x): # x:[32, 96, 7]
        if self.output_attention:
            return self.dft_forward(x)

        b, t, d = x.shape                           # 32, 96, 7
        x_freq = fft.rfft(x, dim=1)                 # [32, 49, 7]

        if t % 2 == 0:
            x_freq = x_freq[:, self.low_freq:-1]    # [32, 47, 7]
            f = fft.rfftfreq(t)[self.low_freq:-1]   # [47]
        else:
            x_freq = x_freq[:, self.low_freq:]
            f = fft.rfftfreq(t)[self.low_freq:]

        x_freq, index_tuple = self.topk_freq(x_freq)                            # [32, 3, 7]
        f = repeat(f, 'f -> b f d', b=x_freq.size(0), d=x_freq.size(2)).to(x_freq.device)  # [32, 47, 7]
        f = rearrange(f[index_tuple], 'b f d -> b f () d').to(x_freq.device)    # [32, 3, 1, 7]

        return self.extrapolate(x_freq, f, t), None

    def extrapolate(self, x_freq, f, t):
        x_freq = torch.cat([x_freq, x_freq.conj()], dim=1)      # [32, 6, 7]
        f = torch.cat([f, -f], dim=1)   # [32, 6, 1, 7]
        t_val = rearrange(torch.arange(t + self.pred_len, dtype=torch.float),'t -> () () t ()').to(x_freq.device)
        # [1, 1, 96, 1]
        amp = rearrange(x_freq.abs() / t, 'b f d -> b f () d')  # [32, 6, 1, 7]
        phase = rearrange(x_freq.angle(), 'b f d -> b f () d')  # [32, 6, 1, 7]
        x_time = amp * torch.cos(2 * math.pi * f * t_val + phase)   # [32, 6, 96, 7]
