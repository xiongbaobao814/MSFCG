...
class MyModel(nn.Module):  
    def __init__(self, configs, scale):  
        ...
        self.nonlin_map2 = nn.Sequential(  
                nn.Linear(self.scale, 2*self.hidden_dim),  
                nn.BatchNorm1d(2*self.hidden_dim)  
            )  
    def forward(self, x):
        x = self.nonlin_map2(x)
        return x 


# 定义一个用于提取特征的一维卷积神经网络模型，通过多层卷积和池化操作逐步提取并浓缩输入信号中的特征信息
class Feature_extractor_1DCNN(nn.Module):
    def __init__(self, input_channels, num_hidden, embedding_dimension, kernel_size=3, dropout=0):
        ...
        # num_hidden*2 → embedding_dimension: 96 → 16
...
class Dot_Graph_Construction_weights(nn.Module):
    ...
    def forward(self, node_features):                           # [32, 7, 32]  [32, 14, 32]
        ...
        node_features = self.mapping(node_features)             # [32, 7, 32]  [32, 14, 32]
        # node_features = F.leaky_relu(node_features)
        bs, N, dimen = node_features.size()                     # 32, 7, 32
        # 计算节点特征之间的点积，得到邻接矩阵
        node_features_1 = torch.transpose(node_features, 1, 2)  # [32, 32, 7]  [32, 32, 14]
        Adj = torch.bmm(node_features, node_features_1)         # [32, 7, 7]   [32, 14, 14]
...
<<<<<<< Tabnine <<<<<<<
    def __init__(self, num_nodes, input_dim, output_dim, k):#-
        # k表示不同的邻居组合方式，即“感受野”的大小。k控制了邻接矩阵A的累积次数，从而决定每个节点可以考虑多少层邻居的信息#-
        # k=1 means the traditional GCN#-
    def __init__(self, num_nodes: int, input_dim: int, output_dim: int, k: int):#+
        """#+
        Initialize a Graph Convolutional Neural Network (GCN) module.#+
#+
        Parameters:#+
        num_nodes (int): The number of nodes in the graph.#+
        input_dim (int): The dimension of the input features.#+
        output_dim (int): The dimension of the output features.#+
        k (int): The number of different neighborhood combinations to consider.#+
#+
        The module uses a traditional GCN approach, where each node's representation is a weighted sum of its neighbors' representations.#+
        The number of layers in the GCN is determined by the value of k.#+
        """#+
        super(MPNN_mk_v2, self).__init__()
        self.way_multi_field = 'sum'    # 决定多个感受野的结果是拼接还是相加,'cat'(concatnate) or 'sum'(sum up) 
        self.num_nodes = num_nodes      # 7
...
>>>>>>> Tabnine >>>>>>># {"conversationId":"9a8c03da-2496-499a-aa06-0fad3f4c32ad","source":"instruct"}

    def forward(self, X, A, num_windows, moving_window):    # [32, 14, 32], [32, 14, 14]
        ...
        if self.way_multi_field == 'cat':            # 组合不同感受野的结果
            ...
            GCN_output_ = sum(GCN_output_)                  # [32, 14, 16]
        
        # 图卷积结果的批量归一化
        GCN_output_ = torch.transpose(GCN_output_, -1, -2)
        GCN_output_ = self.bn1(GCN_output_)
        GCN_output_ = torch.transpose(GCN_output_, -1, -2)  # [32, 14, 16]
        GCN_output_ = F.leaky_relu(GCN_output_)             # 引入非线性
        
        # 图池化操作
        bs, _, _ = GCN_output_.size()
        GCN_output = torch.reshape(GCN_output_, [bs, num_windows, moving_window, self.num_nodes, self.output_dim])  
        # [32, 1, 2, 7, 16] [32, 2, 2, 7, 16]
        
        if self.pool_choice == 'mean':
            GPooling_output = torch.mean(GCN_output, 2)              # [32, 1, 7, 16] [32, 2, 7, 16]
        ...
        return GPooling_output


# 将原始时间序列转换成一系列特定大小的时间滑动窗口
def WindowConv_Graph(input, moving_window, stride):                          # [32, 1, 7, 32]  [32, 2, 7, 32]   
...
class GraphConvpoolMPNN_block(nn.Module):
    def __init__(self, input_dim, output_dim, num_nodes, scale_num, moving_window, stride, decay, pool_choice):
        ...
        # 构造图邻接矩阵
        self.graph_construction = Dot_Graph_Construction_weights_v2(input_dim, 14)
        self.BN = nn.BatchNorm1d(input_dim)
        # 计算衰减矩阵
        self.pre_relation = Mask_Matrix(num_nodes, moving_window, decay)    # 固定值的矩阵
    ...
    def forward(self, input):   # [32, 1, 7, 32]  [32, 2, 7, 32]
        ...
        print("input_con: ", input_con.shape)
        bs, num_windows, num_nodes, moving_window, feature_dim = input_con.size()   # [32, 1, 7, 1, 32] [32, 1, 7, 2, 32]
        input_con_ = torch.transpose(input_con, 2, 3)                               # [32, 1, 1, 7, 32] [32, 1, 2, 7, 32]
        input_con_ = torch.reshape(input_con_, [bs*num_windows, moving_window*num_nodes, feature_dim]) # [32, 7, 32]  [32, 14, 32]

        # 构造图邻接矩阵A_input:衰减前的图邻接矩阵与衰减后的图邻接矩阵
        Adj_nodecay = self.graph_construction(input_con_)  # [32, 7, 7]  [32, 14, 14]
        Adj_decay = Adj_nodecay*self.pre_relation          # [32, 7, 7]  [32, 14, 14]

        # 消息传递：图卷积与池化操作
        input_con_ = torch.transpose(input_con_, -1, -2)   # [32, 32, 7]  [32, 32, 14]
        input_con_ = self.BN(input_con_)
        input_con_ = torch.transpose(input_con_, -1, -2)   # [32, 7, 32]  [32, 14, 32]
        X_output = self.MPNN(input_con_, Adj_decay, num_windows = num_windows, moving_window = moving_window)        # [32, 7, 16]  [32, 14, 16] [64, 14, 16]
        # X_output = torch.reshape(X_output, [bs, num_windows, moving_window, num_nodes, self.output_dim])  
        # # [32, 1, 2, 7, 16] [32, 2, 2, 7, 16]
        
        # # 图池化操作
        # if self.pool_choice == 'mean':
        #     X_output = torch.mean(X_output, 2)              # [32, 1, 7, 16] [32, 2, 7, 16]
        # elif self.pool_choice == 'max':
        ...
        # 图池化操作
        if self.pool_choice == 'mean':
            X_output = torch.mean(X_output, 1)              # [32, 1, 7, 16] [32, 1, 7, 16]
        elif self.pool_choice == 'max':
            X_output, ind = torch.max(X_output, 1)
        else:
            print('input choice for pooling cannot be read')
