...
class MPNN_mk_v2(nn.Module):
    ...
    def forward(self, X, A):    # [32, 14, 32], [32, 14, 14]
        ...
        for i in range(self.k):
            ...
            out_k = self.theta[i](torch.bmm(A_, X))  # 线性变换 [32, 14, 16]
...
class GraphConvpoolMPNN_block(nn.Module):
    def __init__(self, input_dim, output_dim, num_nodes, scale_num, moving_window, stride, decay, pool_choice):
        ...
        self.pre_relation = Mask_Matrix(num_nodes, scale_num, decay)    # 固定值的矩阵
        # 执行消息传递和图卷积操作
    ...
    def forward(self, input):   # [32, 1, 7, 32]  [32, 2, 7, 32]
        ...
        bs, scale_num, num_nodes, feature_dim = input.size()
        input_con_ = torch.reshape(input, [bs, scale_num*num_nodes, feature_dim]) # [32, 7, 32]  [32, 14, 32]
        ...
        X_output = torch.reshape(X_output, [bs, scale_num, num_nodes, self.output_dim])  
        # [32, 1, 7, 16] [32, 2, 7, 16]
        
        # 图池化操作
        if self.pool_choice == 'mean':
            X_output = torch.mean(X_output, 1)              # [32, 1, 7, 16] [32, 2, 7, 16]
