...
class Dot_Graph_Construction_weights(nn.Module):
    ...
    def forward(self, node_features):                           # [32, 7, 32]  [32, 14, 32]
        ...
        if N == 7:
            ...
            node_features = self.mapping2(node_features)          # [32, 14, 16]# node_features = F.leaky_relu(node_features)
...
class Dot_Graph_Construction_weights_v2(nn.Module):
    ...
    def forward(self, node_features):                           # [32, 7, 32]  [32, 14, 32]
        ...
        if N == 7:
            ...
            node_features = self.mapping2(node_features)          # [32, 14, 16]
        # node_features = F.leaky_relu(node_features)

        # 计算节点特征之间的点积，得到邻接矩阵
        node_features_1 = torch.transpose(node_features, 1, 2)  # [32, 32, 14]  [32, 16, 14]
...
class GraphConvpoolMPNN_block(nn.Module):
    def __init__(self, input_dim, output_dim, num_nodes, scale_num, moving_window, stride, decay, pool_choice):
        ...
        # 构造图邻接矩阵
        self.graph_construction = Dot_Graph_Construction_weights_v2(input_dim, 32)
    ...
    def forward(self, input):   # [32, 1, 7, 32]  [32, 2, 7, 32]
        ...
        # 消息传递:图卷积与池化操作
        input_con_1 = torch.transpose(input_con_1, -1, -2)               # [32, 32, 14]  [32, 16, 14]
        input_con_1 = self.BN(input_con_1)
        input_con_1 = torch.transpose(input_con_1, -1, -2)               # [32, 14, 32]  [32, 14, 16]
        GCNPooling_output = self.MPNN(input_con_1, Adj_decay, input_con) # [32, 7, 16]  [32, 1,, 16] [64, 14, 16]
