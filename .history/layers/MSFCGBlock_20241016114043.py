...
class MPNN_mk_v2(nn.Module):
    ...
    def forward(self, X, A):    # [32, 14, 32], [32, 14, 14]
        ...
        GCN_output_ = F.leaky_relu(GCN_output_)
        return F.leaky_relu(GCN_output_)    # 引入非线性
...
class GraphConvpoolMPNN_block(nn.Module):
    ...
    def forward(self, input):   # [32, 1, 7, 32]  [32, 2, 7, 32]
        ...
        input_nodecay = self.graph_construction(input_con_)       # [32, 7, 7]  [32, 14, 14]
        input_decay = input_nodecay*self.pre_relation                 #             [32, 14, 14]

        # 消息传递与图卷积操作        
        input_con_ = torch.transpose(input_con_, -1, -2)    #             [32, 32, 14]
        input_con_ = self.BN(input_con_)
        input_con_ = torch.transpose(input_con_, -1, -2)    #             [32, 14, 32]
        X_output = self.MPNN(input_con_, input_decay)       #             [32, 14, 16] [64, 14, 16]
        X_output = torch.reshape(X_output, [bs, num_windows, moving_window, num_nodes, self.output_dim])  
        # [100, 1, 2, 9, 16] [32, 1, 2, 7, 16] [32, 2, 2, 7, 16]
