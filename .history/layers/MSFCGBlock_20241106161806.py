...
class Dot_Graph_Construction_weights_v2(nn.Module):
    ...
    def forward(self, node_features):                           # [32, 7, 32]  [32, 14, 32]
        ...
        if N == 7:
            node_features = torch.transpose(node_features, 1, 2)  # [32, 32, 7] [32, 32, 14]
            node_features = self.mapping1(node_features)          # [32, 32, 14]  [32, 14, 32]
            node_features = torch.transpose(node_features, 1, 2)  # [32, 14, 32] [32, 32, 14]
        else:
            node_features = self.mapping2(node_features)          # [32, 32, 14]  [32, 14, 32]
        # node_features = F.leaky_relu(node_features)

        # 计算节点特征之间的点积，得到邻接矩阵
        node_features_1 = torch.transpose(node_features, 1, 2)  # [32, 32, 14]  [32, 32, 14]
        Adj = torch.bmm(node_features, node_features_1)         # [32, 14, 7]   [32, 14, 14]
...
class MPNN_mk_v2(nn.Module):
    def __init__(self, input_dim, output_dim, pool_choice, rf):
        # rf表示不同的邻居组合方式，即“感受野”的大小。rf控制了邻接矩阵A的累积次数，从而决定每个节点可以考虑多少层邻居的信息
        # rf=1 means the traditional GCN
    ...
    def forward(self, X, A, input_con):    # [32, 14, 32], [32, 14, 14]
...
class GraphConvpoolMPNN_block(nn.Module):
    def __init__(self, input_dim, output_dim, num_nodes, scale_num, moving_window, stride, decay, pool_choice):
        ...
        # 计算衰减矩阵
        self.pre_relation = Mask_Matrix(num_nodes, moving_window, decay)    # 固定值的矩阵[14, 14]
        # 执行消息传递和图卷积池化操作
        self.MPNN = MPNN_mk_v2(input_dim, output_dim, pool_choice, rf=2)
        self.pool_choice = pool_choice
    ...
    def forward(self, input):   # [32, 1, 7, 32]  [32, 2, 7, 32]
        ...
        # print("WindowConv_Graph: ", input_con.shape)
        ...
        GCNPooling_output = self.MPNN(input_con_, Adj_decay, input_con) # [32, 7, 16]  [32, 14, 16] [64, 14, 16]
        ...
        # print("GCNPooling_output: ", GCNPooling_output.shape)
