...
class Feature_extractor_1DCNN(nn.Module):
    def __init__(self, input_channels, cnnhidden_channels, cnnout_channels, kernel_size=3, dropout=0):  # 1, 48, 16, 6
        super(Feature_extractor_1DCNN, self).__init__()
        # input_channels → cnnhidden_channels: 1 → 48
        self.conv_block1 = nn.Sequential(
            nn.Conv1d(input_channels, cnnhidden_channels, kernel_size=kernel_size, stride=1, bias=False, padding=(kernel_size//2)),
            nn.BatchNorm1d(cnnhidden_channels),
            nn.ReLU(),
            nn.MaxPool1d(kernel_size=2, stride=2, padding=1),
            nn.Dropout(dropout)
        )
        # cnnhidden_channels → cnnhidden_channels*2: 48 → 96
        self.conv_block2 = nn.Sequential(
            nn.Conv1d(cnnhidden_channels, cnnhidden_channels*2, kernel_size=kernel_size, stride=1, bias=False, padding=2),
            nn.BatchNorm1d(cnnhidden_channels*2),
            nn.ReLU(),
            nn.MaxPool1d(kernel_size=2, stride=2, padding=1)
        )
        # cnnhidden_channels*2 → embedding_dimension: 96 → 16
        self.conv_block3 = nn.Sequential(
            nn.Conv1d(cnnhidden_channels*2, cnnout_channels, kernel_size=kernel_size, stride=1, bias=False, padding=3),
            nn.BatchNorm1d(cnnout_channels),
            nn.ReLU(),
            nn.MaxPool1d(kernel_size=2, stride=2, padding=1),
        )
...
class MPNN_mk_v2(nn.Module):
    ...
    def forward(self, X, A, input_con):    # [32, 16, 14], [32, 14, 14]
        ...
        ## 图卷积结果的批量归一化
        GCN_output_ = torch.transpose(GCN_output_, -1, -2)  # [32, 16, 14]
...
class GraphConvpoolMPNN_block(nn.Module):
    def __init__(self, input_dim, output_dim, num_nodes, moving_window, stride, decay, pool_choice):
    ...
    def forward(self, input):   # [32, 1, 7, 32]  [32, 2, 7, 32]
        ...
        # 消息传递:图卷积与池化操作
        input_con_1 = torch.transpose(input_con_1, -1, -2)               # [32, 32, 14]  [32, 32, 14]
        input_con_1 = self.BN(input_con_1)
        input_con_1 = torch.transpose(input_con_1, -1, -2)               # [32, 14, 32]  [32, 14, 32]
        GCNPooling_output = self.MPNN(input_con_1, Adj_decay, input_con) # [32, 7, 16]  [32, 1, 7, 16] [64, 14, 16]
