...
class MPNN_mk_v2(nn.Module):
    ...
    def forward(self, X, Adj, input_con):    # [32, 14, 32], [32, 14, 14]
        ...
        for i in range(self.field):
            if i == 0:
                A_ = Adj                                      # 初始邻接矩阵, [32, 14, 14]
            else:
                A_ = torch.bmm(Adj_, Adj)                       # 通过多次矩阵乘法累积邻居信息
            out_k = self.theta[i](torch.bmm(Adj_, X))         # 线性变换(图卷积), [32, 14, 16]
...
class GraphConvpoolMPNN_block(nn.Module):
    ...
    def forward(self, input):   # [32, 1, 7, 32]  [32, 2, 7, 32]
        ...
        new_input_con, Adj_nodecay = self.graph_construction(input_con_)  # [32, 14, 32],[32, 14, 14]
        ...
        new_input_con = torch.transpose(new_input_con, -1, -2)               # [32, 14, 32] [32, 14, 32]
        new_input_con_ = torch.reshape(new_input_con, [bs, num_windows, num_nodes, 2, feature_dim])
        GCNPooling_output = self.MPNN(new_input_con, Adj_decay, new_input_con_) # [32, 7, 16]  [32, 1, 7, 16] [64, 14, 16]
