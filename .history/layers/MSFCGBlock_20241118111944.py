...
class Feature_extractor_1DCNN(nn.Module):
    def __init__(self, input_channels, cnnhidden_channels, cnnout_channels, kernel_size=3, dropout=0):  # 1, 48, 16, 6
        ...
        self.conv_block2 = nn.Sequential(
            nn.Conv1d(cnnhidden_dim, cnnhidden_dim*2, kernel_size=kernel_size, stride=1, bias=False, padding=2),
            nn.BatchNorm1d(cnnhidden_dim*2),
            nn.ReLU(),
            nn.MaxPool1d(kernel_size=2, stride=2, padding=1)
        )
        # num_hidden*2 → embedding_dimension: 96 → 16
        self.conv_block3 = nn.Sequential(
            nn.Conv1d(cnnhidden_dim*2, cnnout_dim, kernel_size=kernel_size, stride=1, bias=False, padding=3),
            nn.BatchNorm1d(cnnout_dim),
            nn.ReLU(),
            nn.MaxPool1d(kernel_size=2, stride=2, padding=1),
        )
