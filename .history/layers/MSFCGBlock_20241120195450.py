...
class Dot_Graph_Construction_weights(nn.Module):
    ...
    def forward(self, node_features):                           # [32, 7, 32] [32, 14, 32] [96, 14, 32]
        ...
        bs1, N1, feature_dim1 = node_features.size()
        if N1 == 7:
            ...
            node_features = self.mapping2(node_features)          # [32, 14, 32] [96, 14, 32]
...
class GraphConvpoolMPNN_block(nn.Module):
    ...
    def forward(self, input):   # [32, 1, 7, 32]  [32, 2, 7, 32]
        ...
        bs, num_windows, num_nodes, moving_window, feature_dim = input_con.size()   # [32, 1, 7, 1, 32] [32, 1, 7, 2, 32] [32, 3, 7, 2, 32]
        input_con_ = torch.transpose(input_con, 2, 3)                               # [32, 1, 1, 7, 32] [32, 1, 2, 7, 32]
        input_con_ = torch.reshape(input_con_, [bs*num_windows, moving_window*num_nodes, feature_dim]) # [32, 7, 32] [32, 14, 32] [96, 14, 32]
