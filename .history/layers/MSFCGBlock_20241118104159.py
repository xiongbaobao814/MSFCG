...
class Feature_extractor_1DCNN(nn.Module):
    def __init__(self, input_channels, lstmhidden_dim, lstmout_dim, kernel_size=3, dropout=0):  # 1, 48, 16, 6
        super(Feature_extractor_1DCNN, self).__init__()
        # input_channels → num_hidden: 1 → 48
        self.conv_block1 = nn.Sequential(
            nn.Conv1d(input_channels, lstmhidden_dim, kernel_size=kernel_size, stride=1, bias=False, padding=(kernel_size//2)),
            nn.BatchNorm1d(lstmhidden_dim),
            nn.ReLU(),
        ...
        self.conv_block2 = nn.Sequential(
            nn.Conv1d(lstmhidden_dim, lstmhidden_dim*2, kernel_size=kernel_size, stride=1, bias=False, padding=2),
            nn.BatchNorm1d(lstmhidden_dim*2),
            nn.ReLU(),
            nn.MaxPool1d(kernel_size=2, stride=2, padding=1)
        )
        # num_hidden*2 → embedding_dimension: 96 → 16
        self.conv_block3 = nn.Sequential(
            nn.Conv1d(lstmhidden_dim*2, lstmout_dim, kernel_size=kernel_size, stride=1, bias=False, padding=3),
            nn.BatchNorm1d(lstmout_dim),
            nn.ReLU(),
            nn.MaxPool1d(kernel_size=2, stride=2, padding=1),
        )
