...
class Dot_Graph_Construction_weights(nn.Module):
    def __init__(self, c_out, input_dim):
        super().__init__()
        self.c_out = c_out
        self.input_dim = input_dim
        self.mapping1 = nn.Linear(c_out, c_out*2)
...
class GraphConvpoolMPNN_block(nn.Module):
    def __init__(self, input_dim, output_dim, c_out, moving_window, stride, decay, pool_choice):
        super(GraphConvpoolMPNN_block, self).__init__()
        self.moving_window = moving_window  # 2, 2
        self.stride = stride                # 1, 2

        # 构造图邻接矩阵
        self.graph_construction = Dot_Graph_Construction_weights(c_out, input_dim)
        self.BN = nn.BatchNorm1d(input_dim)
        # 计算衰减矩阵
        self.pre_relation = Mask_Matrix(c_out, moving_window, decay)    # 固定值的矩阵[14, 14]
        # 执行消息传递和图卷积池化操作
