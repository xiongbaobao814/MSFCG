...
class GraphBlock2(nn.Module):
    def __init__(self, c_out, d_model, conv_channel, skip_channel, gcn_depth, dropout, propalpha, seq_len, node_dim):
        super(GraphBlock2, self).__init__()

        # 定义两个可学习的参数矩阵
        self.nodevec1 = nn.Parameter(torch.randn(c_out, node_dim), requires_grad=True)  # [7,10]
        self.nodevec2 = nn.Parameter(torch.randn(node_dim, c_out), requires_grad=True)  # [10,7]
        self.start_conv = nn.Conv2d(1, conv_channel, (d_model - c_out + 1, 1))
        self.gconv1 = mixprop(conv_channel, skip_channel, gcn_depth, dropout, propalpha) # 图卷积
        self.gelu = nn.GELU()
    ...
    def forward(self, x):
        # 计算自适应邻接矩阵adj
        adp = F.softmax(F.relu(torch.mm(self.nodevec1, self.nodevec2)), dim=1)  # [7, 7]
        out = x.unsqueeze(1).transpose(2, 3)    # [32, 1, 512, 96]
        out = self.start_conv(out)              # [32, 32, 7, 96]
        
        # 图卷积mixprop
        out = self.gelu(self.gconv1(out, adp))  # [32, 32, 7, 96]
        out = self.end_conv(out).squeeze()      # [32, 96, 7]
        out = self.linear(out)                  # [32, 96, 512]
        return self.norm(x + out)


# MixHop 图网络的多跳学习(paper:MixHop: Higher-Order Graph Convolutional Architectures via Sparsified Neighborhood Mixing)
class mixprop(nn.Module):
    def __init__(self, c_in, c_out, gdep, dropout, alpha):
        super(mixprop, self).__init__()
        self.nconv = nconv()                    # 用于节点特征传播
        self.mlp = linear((gdep+1)*c_in,c_out)  # 用于整合多层特征输出
        self.gdep = gdep
        self.dropout = dropout
        self.alpha = alpha

    def forward(self, x, adj):
        adj = adj + torch.eye(adj.size(0)).to(x.device)     # 对邻接矩阵添加单位矩阵并进行行归一化处理
        d = adj.sum(1)              # [7]
        h = x                       # 初始化节点特征h为输入特征x, [32, 32, 7, 96]
        out = [h]
        a = adj / d.view(-1, 1)     # 对邻接矩阵adj进行归一化
...
class simpleVIT(nn.Module):
    def __init__(self, in_channels, emb_size, patch_size=2, depth=1, num_heads=4, dropout=0.1,init_weight =True):
        ...
        for _ in range(self.depth):
            self.layers.append(nn.ModuleList([nn.LayerNorm(emb_size),
