...
def WindowConv_Graph(input, moving_window, stride):                          # [32, 1, 7, 32]  [32, 2, 7, 32]   
    ...
    # if scale_num == 1:
    #     y_ = F.unfold(x_, (num_nodes, 1), stride=1)                          # [32, 224, 1]    [32, 448, 1]
    #     y_ = torch.reshape(y_, [bs, feature_dim, num_nodes, scale_num, -1])  # [32, 32, 7, 1, 1]   [32, 32, 7, 2, 1]
    # else:
    #     y_ = F.unfold(x_, (num_nodes, moving_window), stride=stride)
    #     y_ = torch.reshape(y_, [bs, feature_dim, num_nodes, moving_window, -1]) # [32, 32, 7, 2, 1]   [32, 32, 7, 2, 1]
...
class GraphConvpoolMPNN_block(nn.Module):
    ...
    def forward(self, input):   # [32, 1, 7, 32]  [32, 2, 7, 32]
        ...
        if self.pool_choice == 'mean':
            X_output = torch.mean(X_output, 2)              # [100, 1, 9, 16]   [32, 1, 7, 16]
