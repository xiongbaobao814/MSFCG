...
class GraphBlock(nn.Module):
    def __init__(self, configs):
        ...
        # 图构建和聚合：图卷积池化MPNN模块
        self.MPNN1 = GraphConvpoolMPNN_block(2*self.hidden_dim, self.hidden_dim, self.c_out,  
                                                moving_window=self.moving_windows[0], stride=self.strides[0], 
                                                decay=self.decay, pool_choice=self.pooling_choice)
        self.MPNN2 = GraphConvpoolMPNN_block(2*self.hidden_dim, self.hidden_dim, self.c_out, 
...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.layer_norm = nn.LayerNorm(self.c_out)
        
        self.projection = nn.Linear(self.c_out, self.num_nodes, bias=True)
    ...
    def forward(self, x_enc, x_mark_enc, mask=None): # [32, 96, 7], [32, 96, 4]
        ...
        # out = self.seasonality_and_trend_decompose(x_enc)   # [32, 96, 7]
        
        # # 2.编码
        # out = self.enc_embedding(out, x_mark_enc)     # [32, 96, 512]        
        # out = self.embed_fc(out)                    # [32, 96, 7]

        # 3.多尺度图处理, 包含layer个MultiScaleGraphBlock实例
        for i in range(self.layer):   # 2层
            out = self.seasonality_and_trend_decompose(out)
            out = self.enc_embedding(out, x_mark_enc)
            out = self.embed_fc(out)
            
            layer_out = self.model[i](out)              # [32, 96, 7]
            x_enc = self.layer_norm(layer_out)            # [32, 96, 7], 归一化
        
        # 4.投影
        out = self.projection(x_enc)                            # [32, 96, 7]
