...
class MyModel(nn.Module):  
    def __init__(self, configs, scale):  
        ...
        self.nonlin_map2 = nn.Sequential(  
                nn.Linear(224, 2*self.hidden_dim),  
                nn.BatchNorm1d(2*self.hidden_dim)  
            )  
    def forward(self, x):  
        # 假设x的形状是(batch_size, lstmout_dim * conv_out)，这是nn.Linear层所期望的  
        # 但实际上，你可能需要从LSTM和CNN的输出中重塑x来获得这个形状  
        # 这里为了简化，我们直接假设x已经是正确的形状  
        return self.nonlin_map(x)  
