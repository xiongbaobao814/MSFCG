...
class GraphBlock(nn.Module):
    def __init__(self, configs, scale_num, scale):
        ...
        self.nonlin_map2 = nn.Sequential(nn.Linear(self.lstmout_dim*self.conv_out, 2*self.hidden_dim),
                                        nn.BatchNorm1d(2*self.scale))   # 128->32
...
class ScaleGraphBlock(nn.Module):
    def __init__(self, configs):
        ...
        for i in range(self.k):
            self.gconv.append(GraphBlock(configs, scale=self.patch_size_list[i]))
    ...
    def forward(self, x_enc):
        ...
        for i in range(self.k):
            ...
            # 构造多尺度图
            for i in range(self.k):
                self.gconv.append(GraphBlock(scale=self.patch_size_list[i], scale_num = length // scale))
