...
class Model(nn.Module):
    ...
    def forward(self, x_enc, x_mark_enc, mask=None):    # [32, 96, 7], [32, 96, 4]
        ...
        new_x_enc = self.seasonality_and_trend_decompose(x_enc)   # [32, 96, 7]
        
        # 2.编码
        out = self.enc_embedding(new_x_enc, x_mark_enc)     # [32, 96, 512]
        # x_input = self.embed_fc(x_input)                    # [32, 96, 7]

        # 3.多尺度图处理, 包含layer个MultiScaleGraphBlock实例
        for i in range(self.layer):   # 2层
            layer_out = self.model[i](new_x_enc)              # [32, 96, 7]
            x_enc = self.layer_norm(layer_out)            # [32, 96, 7], 归一化
        
        # 4.投影
        out = self.projection1(x_enc)                           # [32, 96, 7]        
