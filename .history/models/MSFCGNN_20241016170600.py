...
class GraphBlock(nn.Module):
    def __init__(self, configs):
        ...
        self.fc1 = nn.Sequential(OrderedDict([   # 16x1x7,16 
            ('fc1', nn.Linear(self.hidden_dim, self.hidden_dim)),
            ('relu1', nn.ReLU(inplace=True)),
            ('fc2', nn.Linear(self.hidden_dim, self.hidden_dim)),
            ('relu2', nn.ReLU(inplace=True)),
            ('fc3', nn.Linear(self.hidden_dim, self.seq_len)),
        ]))
    ...
    def forward(self, x_enc):
        ...
        # # Graph Convolution
        # feature = self.MPNN1(A_input_)                              # [32, 1, 7, 16]
        # feature = torch.reshape(feature, [bs, num_nodes, -1])       # [32, 7, 16]        
        # print(feature.shape)
        # feature = self.fc(feature)                                 # [32, 7, 96]
        # feature = torch.transpose(feature, 1, 2)
        return feature
...
class Model(nn.Module):
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None):
        ...
        print('len(outputs) outputs shape is {}'.format(len(outputs)))
        print(outputs)
        return outputs
