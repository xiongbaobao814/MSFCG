...
class GraphBlock(nn.Module):
    def __init__(self, configs):
        ...
        # 图构建和聚合：图卷积池化MPNN模块
        self.MPNN1 = GraphConvpoolMPNN_block(2*self.hidden_dim, self.hidden_dim, self.num_nodes,  
                                                moving_window=self.moving_windows[0], stride=self.strides[0], 
                                                decay=self.decay, pool_choice=self.pooling_choice)
        self.MPNN2 = GraphConvpoolMPNN_block(2*self.hidden_dim, self.hidden_dim, self.num_nodes, 
        ...
        self.fc = nn.Sequential(OrderedDict([   # 16x2x7,16x2 
            ...
            ('fc3', nn.Linear(self.lstmhidden_dim, 2*self.lstmhidden_dim)),
        ]))
...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.projection = nn.Linear(self.enc_in, self.# The `c_out` parameter in the code is used to
        # specify the output dimension of the model. It
        # determines the number of output channels or
        # features that the model will produce. In the
        # context of the provided code, `c_out` is used
        # in various parts of the model architecture to
        # define the size of the output tensors at
        # different stages of processing.
        # The `c_out` parameter in the code is used to
        # specify the output dimension of the model. It
        # determines the number of output channels or
        # features that the model will produce. In the
        # context of the provided code snippet, `c_out`
        # is used in various parts of the model
        # architecture to define the size of the output
        # tensors at different stages of processing.
        c_out, bias=True)
        self.seq2pred = Project(self.individual, self.c_out, self.seq_len, self.pred_len, self.dropout)
        
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None):    # [32, 96, 7], [32, 96, 4]
        ...
        # # x_enc = self.revin_fc(x_input)                    # [32, 96, 512]
        ...
        # 4.投影
        # out = self.projection(outputs)                              # [32, 96, 7]
        out = self.seq2pred(layer_out.transpose(1, 2)).transpose(1, 2)  # [32, 96, 7]
