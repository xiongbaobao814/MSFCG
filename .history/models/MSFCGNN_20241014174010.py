...
class MutiScaleBlock(nn.Module):
    def __init__(self, configs):
        super(MutiScaleBlock, self).__init__(configs)        
        ...
        # 非线性映射模块，用于特征提取
        self.graphblock = GraphBlock(configs)
...
class MyModel(nn.Module):  
    def __init__(self, configs, scale):  
        super(MyModel, self).__init__(configs)  
        self.lstmout_dim = configs.lstmout_dim  
        self.conv_out = configs.conv_out
        self.hidden_dim = configs.hidden_dim
        self.scale = scale  
...
class GraphBlock(nn.Module):
    def __init__(self, configs):
        super(GraphBlock, self).__init__(configs)        
        self.num_nodes = configs.num_nodes
        self.seq_len = configs.seq_len
        self.d_model = configs.d_model
    ...
    def forward(self, x_enc):
        ...
        mymodel = MyModel(self.configs, scale)
...
class ScaleGraphBlock(nn.Module):
    def __init__(self, configs):
        super(ScaleGraphBlock, self).__init__(configs)        
        self.num_nodes = configs.num_nodes
        self.seq_len = configs.seq_len
        self.pred_len = configs.pred_len
        self.k = configs.top_k
