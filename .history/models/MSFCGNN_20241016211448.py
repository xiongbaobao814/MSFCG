...
class MultiScaleGraphBlock(nn.Module):
    ...
    def forward(self, x_enc):
        bs, dimension, num_nodes = x_enc.size()                     # [32, 96, 7]                
        scale_list, scale_weight = FFT_for_Period(x_enc, self.k)    # scale_weight:[32, 5]
        multiscale_outputs = []                                                # scale_list:[96, 48, 32, 24, 19]
        for i in range(self.k-1):
        ...
        multiscale_outputs = torch.stack(multiscale_outputs, dim=-1)              # [32, 96, 512, 5]
        
        # adaptive aggregation
        scale_weight = F.softmax(scale_weight, dim=1)                               # [32, 5]
        scale_weight = scale_weight.unsqueeze(1).unsqueeze(1).repeat(1, dimension, num_nodes, 1)    # [32, 96, 512, 5]
...
class Model(nn.Module):
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None):
        ...
        print('len(outputs) is {}'.format(len(outputs)))
        print(outputs)
        return outputs
