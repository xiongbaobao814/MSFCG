...
class MultiScaleGraphBlock(nn.Module):
    ...
    def forward(self, x_enc):
        ...
        outputs = torch.stack(outputs, dim=-1)  # [32, 96, 512, 4]
...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        # # 仿MSGNet逻辑
        self.enc_embedding = DataEmbedding(configs.enc_in, self.d_model, configs.embed, configs.freq, configs.dropout)
        self.layer = configs.e_layers
        self.model = nn.ModuleList([MultiScaleGraphBlock(configs) for _ in range(configs.e_layers)])
        ...
        # self.mutiscalegraph = MultiScaleGraphBlock(configs)
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None):
        ...
        # outputs = self.mutiscalegraph(x_enc)
        # print('shape of outputs is {}'.format(outputs.shape))
        # print(outputs)

        for i in range(self.layer):   # 2层,共包含2个ScaleGraphBlock实例
            layer_out = self.model[i](x_enc)     # [32, 96, 512]
            outputs = self.layer_norm(layer_out) # [32, 96, 512],归一化
