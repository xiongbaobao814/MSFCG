...
class MutiScaleBlock(nn.Module):
    def __init__(self, args):
        super(MutiScaleBlock, self).__init__(args)        
        ...
        self.lstmhidden_dim = args.lstmhidden_dim
        self.lstmout_dim = args.lstmout_dim
        self.conv_kernel = args.conv_kernel
        self.hidden_dim = args.hidden_dim
        self.time_length = args.time_denpen_len
        self.num_windows = args.num_windows
        self.moving_windows = args.moving_windows
        self.stride = args.stride
        self.decay = args.decay
        self.pooling_choice = args.pooling_choice
        
        # 非线性映射模块，用于特征提取
        self.graphblock = GraphBlock(args)
    ...
    def forward(self, x_enc):
        ...
        for i in range(self.k):
            ...
            # 构造多尺度图
            features, A_input_map1 = self.graphblock(out, scale_num, scale)
        return scale_list
...
class MyModel(nn.Module):  
    ...
    def forward(self, x):  
        x = x.to(self.device)
        return self.nonlin_map2(x)  
    
    
class GraphBlock(nn.Module):
    def __init__(self, args):
        super(GraphBlock, self).__init__(args)        
        self.num_nodes = args.num_nodes
        self.seq_len = args.seq_len
        self.d_model = args.d_model
    ...
    def forward(self, x_enc):
        ...
        mymodel = MyModel(self.args, scale)
        # A_input_ = A_input_.float().to(self.device)
        A_input_map2 = mymodel.nonlin_map2(A_input_)                              # [224, 32]      [448,32]
...
class ScaleGraphBlock(nn.Module):
    def __init__(self, args):
        super(ScaleGraphBlock, self).__init__(args)        
        self.num_nodes = args.num_nodes
        self.seq_len = args.seq_len
        self.pred_len = args.pred_len
        self.k = args.top_k
        ...
        for i in range(self.k):
            self.gconv.append(GraphBlock(args))
    ...
    def forward(self, x_enc):
        ...
        for i in range(self.k):
            ...
            out = out.reshape(bs, scale_num, scale, num_nodes) # [32, 1, 96, 7]
            
            # 构造多尺度图
            output = self.gconv[i](out)
...
class Model(nn.Module):
    def __init__(self, args):
        ...
        # self.model = nn.ModuleList([ScaleGraphBlock(args) for _ in range(args.e_layers)])
        # self.layer_norm = nn.LayerNorm(args.d_model)
        # self.predict_linear = nn.Linear(self.seq_len, self.pred_len + self.seq_len)
        # self.projection = nn.Linear(args.d_model, args.c_out, bias=True)
        # # self.seq2pred = Predict(args.individual ,args.c_out,args.seq_len, args.pred_len, args.dropout)
        ...
        # 多周期尺度提取
        self.scaleset = MutiScaleBlock(args)
        self.scalegraph = ScaleGraphBlock(args)
...
#         A_input_ = self.nonlin_map(A_input)                                  # [224, 18, 14]  [448,16,8]
#         A_input_map1 = A_input_
        
#         A_input_ = torch.reshape(A_input_, [bs*scale_num*num_nodes, -1])     # [224, 252]     [448,128]        
#         A_input_ = self.nonlin_map2(A_input_)                                # [224, 32]      [448,32]
#         A_input_ = torch.reshape(A_input_, [bs, scale_num, num_nodes, -1])   # [32, 1, 7, 32] [32,2,7,32]
# # 672x96 and 128x32
#         A_input_map2 = A_input_
