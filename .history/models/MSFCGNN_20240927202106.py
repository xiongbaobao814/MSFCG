...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        # 图构建和聚合：图卷积池化MPNN模块
        self.MPNN1 = GraphConvMPNN_block(2*self.hidden_dim, self.hidden_dim, self.num_nodes, self.time_length, 
                                                moving_window=self.moving_windows[0], stride=self.stride[0], 
                                                decay=self.decay, pool_choice=self.pooling_choice)
        self.MPNN2 = GraphConvMPNN_block_v6(2*self.hidden_dim, self.hidden_dim, self.num_nodes, self.time_length, 
    ...
    # def nonlin_map2(self, scale):
    #     return nn.Sequential(nn.Linear(self.lstmout_dim*self.conv_out, scale), nn.BatchNorm1d(scale))
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None):
        ...
        scale =scale_list[1]
        A_input = scale_sizes[1]
        scale_num = scale_nums[1]
