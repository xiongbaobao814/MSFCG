    ...
    def forward(self, x_enc):
        ...
        for i in range(self.k):
            ...
            if (self.seq_len) % scale != 0:
                ...
                padding = torch.zeros([x_enc.shape[0], (length - (self.seq_len)), x_enc.shape[2]]).to(x.device)
                out = torch.cat([x_enc, padding], dim=1)
            else:
                length = self.seq_len
                out = x                                         # [32, 96, 7]
            scale_num = length // scale
            out = out.reshape(B, length // scale, scale, N)     # [32, 1, 96, 7]
    ...
    def forward(self, x):
        ...
        for i in range(self.k):
            ...
            if (self.seq_len) % scale != 0:
                ...
                length = self.seq_len
                out = x     # [32, 96, 512]
            out = out.reshape(B, length // scale, scale, N)         # [32, 1, 96, 512]、[32, 2, 48, 512]、[32, 6, 19, 512]   
        ...
        scale_weight = scale_weight.unsqueeze(1).unsqueeze(1).repeat(1, T, N, 1)    # [32, 96, 512, 5]
        res = torch.sum(res * scale_weight, -1)     # [32, 96, 512]
