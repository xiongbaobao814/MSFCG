...
class MultiScaleGraphBlock(nn.Module):
    ...
    def forward(self, x_enc):
        ...
        for i in range(self.k):
            ...
            out = x_enc.reshape(bs, scale_num, scale, num_nodes)       # [32, 1, 96, 7] [32, 2, 48, 7] [32, 3, 32, 7]
            
            ## 构造多尺度图
            out = self.gconv[i](out)         # [32, 96, 7]
            outputs.append(out)
            
            out = x_enc.reshape(bs, scale_num, scale, num_nodes)
            x_enc = out
...
class Model(nn.Module):
    ...
    def forward(self, x_enc, x_mark_enc, mask=None):    # [32, 96, 7], [32, 96, 4]
        ...
        # 4.投影
        outputs = self.projection1(x_input)                           # [32, 96, 7]        
