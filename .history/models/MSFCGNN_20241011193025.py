...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.nonlin_map2 = nn.Sequential(nn.Linear(self.lstmout_dim*self.conv_out, 2*self.hidden_dim),
                                        nn.BatchNorm1d(2*self.hidden_dim))   # 180 → 32, 144->32
        self.enc_embedding = DataEmbedding(configs.enc_in, configs.d_model, configs.embed, configs.freq, configs.dropout)
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None):
        ...
        A_input_ = self.nonlin_map(A_input)                                  # [224, 18, 14] [448,18,8]
        A_input_ = torch.reshape(A_input_, [bs*scale_num*num_nodes, -1])     # [224, 252]    [448,144]        
...
# 672x108 and 144x32
        ...
        X_ = torch.reshape(X_, [bs, num_nodes, scale_num, -1])       # [32, 7, 1, 32]
        X_ = torch.transpose(X_, 1, 2)
